...
#USAGE: RNA_1_attributeBC.pl FILENAME.seq > OUTPUTFILENAME.seq  | FILENAME.seq contains sequencing reads/n
...
#		my @sub_seq=split (/ATAATGGTTACAAATAAAGC|AATAGCATCA/, $SEQ);
		...
		if ($frag_length == 36) 								
			...
			#Stringent if ($separator1 eq $separator2 && $separator2 eq $separator3 && $separator3 eq 'TAGG')		
				if (($separator1 eq $separator2) && ($separator2 eq'TAGG'))
				{$gene ='GFP';$GFP_sep ++;}
			#Stringent: if ($separator1 eq $separator2 && $separator2 eq $separator3 && $separator3 eq 'CGAA')
				if (($separator1 eq $separator2) && ($separator2 eq 'CGAA'))
				{$gene ='mCherry';$mcherry_sep ++;}			
			#print "$gene\t$separator1\t$barcode1\t$separator2\t$barcode2\t$separator3\n";
